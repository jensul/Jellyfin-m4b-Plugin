...
3. **Configure the `manifest.json` file**: Use the enclosed `manifest.json`. Edit the paths where it says `YOUR_GITHUB_USERNAME` and `YOUR_GITHUB_REPO`. 
4. Calculate the MD5 Hash of the published `M4bEnhancer.zip` and add it to `checksum`.
5. Point your Jellyfin Server to this plugin! Navigate to **Dashboard -> Plugins -> Repositories** and click Add. Give it a name and use the `Raw` URL to your GitHub `manifest.json` file. Jellyfin will then beautifully present the plugin for automatic web installation dynamically.
